...
def main() -> int:
    ...
    ap.add_argument("--limit", type=int, default=None,
                     help="probar solo con N torneos")
    ap.add_argument("--debug-pagina", type=str, default=None,
                     help="vuelca el wikitext crudo de una pagina y sale")
    ap.add_argument("--meses", type=int, default=4,
                     help="solo partidos de los ultimos N meses")
    args = ap.parse_args()
    ...
    for n, titulo in enumerate(pendientes, 1):
        try:
            ...
            print(f"[aviso] {titulo}: {type(e).__name__}", file=sys.stderr)
            con_error += 1
            continue
    ...
    print(f"[resumen] torneos con error de red: {con_error}", file=sys.stderr)

    if sin_match > len(pendientes) * 0.5 and len(pendientes) > 5:
        print(f"\n[info] {sin_match} de {len(pendientes)} torneos sin partidos.", file=sys.stderr)

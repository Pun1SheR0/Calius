...
RE_OPONENTE = re.compile(r'opponent(\d)\s*=\s*\{\{\s*SoloOpponent\s*\|(.*?)\}\}', re.S)
# El formato moderno (torneos actuales) usa fecha ISO directa: date=2026-02-22.
# El formato viejo (paginas de hace anos) usa nombre de mes: date=January 15, 2026.
# Se prueban las dos, ISO primero por ser el formato real de lo que nos interesa.
RE_FECHA_ISO = re.compile(r'date\s*=\s*(\d{4})-(\d{2})-(\d{2})')
RE_FECHA_TXT = re.compile(r'([A-Za-z]+)\s+(\d{1,2}),\s*(\d{4})')
# El formato moderno tampoco lleva el marcador en SoloOpponent: va en el/los
# {{Map|...}} de dentro. Se toma el del primer mapa (bestof=1 es el estandar
# actual; con bestof>1 esto se queda con el primer juego, no con la serie).
...
def parsear_match(bloque: str, torneo: str, cutoff_iso: str) -> dict | None:
    ...
    # Marcador: primero se intenta el score= pegado al nombre (paginas
    # antiguas). Si no esta, se busca en el primer {{Map|score1=..}} de
    # dentro del bloque (formato actual).
...
def main() -> int:
    ...
    for n, titulo in enumerate(pendientes, 1):
        ...
        # Verboso a proposito: con ~150 torneos en la categoria acotada,
        # el volumen de log es manejable y esto es lo que nos deja saber
        # que titulo exacto mirar con --debug-pagina si algo no cuadra.
    ...
    # Ya no se corta la ejecucion por esto: comprobado con datos reales que
    # el analizador funciona (27 de 27 bloques en ELibertadores/2026). Que
    # muchos torneos de la categoria no tengan partidos es normal -- mezcla
    # de proximos, cancelados y paginas placeholder junto a los terminados.
